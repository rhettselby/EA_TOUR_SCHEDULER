...
// Normalize it so the rest of the code always works with an array.
function getGuestNames(tour) {
if (Array.isArray(tour.guest_name)) return tour.guest_name;
if (tour.guest_name) return [tour.guest_name]; // graceful fallback
return [];
}
...
// Spread all guest names from the JSONField array, deduplicating
const names = getGuestNames(tour);
names.forEach((name) => {
...
// Prefer the aggregated guests list from groupByStartTime; fall back to the
// tour's own guest_name JSONField array; last resort: ["Guest"]
const guestList =
