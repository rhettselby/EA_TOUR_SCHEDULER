...
def run_slack_agent(channel, text, time_stamp):
    try:
        ...
        query = f"""GENERAL SLACK REPLY TASK: A message was sent in slack at {time_stamp}. Please respond to the following message in channel {channel}: {text}
            Only respond to the message if it is relating to tour coordination/scheduling, the message directly references the slack/tour/bot agent, or the message is part of a longer conversation
